...
describe('MediaService', () => {
  ...
  beforeEach(() => {
    ...
    (s3Config.deleteFromS3 as jest.Mock).mockResolvedValue(undefined);
    (s3Config.getDownloadSignedUrl as jest.Mock).mockResolvedValue('https://signed-url.example.com');
    (s3Config.getUploadSignedUrl as jest.Mock).mockResolvedValue('https://upload-url.example.com');
  ...
  describe('uploadMedia', () => {
    ...
    it('should upload JPEG image successfully', async () => {
      ...
      const result = await mediaService.uploadMedia(file, MediaType.IMAGE, tenantId, userId);
    ...
    it('should upload PDF document successfully', async () => {
      ...
      const result = await mediaService.uploadMedia(file, MediaType.DOCUMENT, tenantId, userId);
    ...
    it('should upload MP4 video successfully', async () => {
      ...
      const result = await mediaService.uploadMedia(file, MediaType.VIDEO, tenantId, userId);
    ...
    it('should reject invalid file extension for image type', async () => {
      ...
      await expect(
        mediaService.uploadMedia(file, MediaType.IMAGE, tenantId, userId)
    ...
    it('should reject file exceeding size limit for videos (16MB)', async () => {
      ...
      await expect(
        mediaService.uploadMedia(file, MediaType.VIDEO, tenantId, userId)
      ).rejects.toThrow(BadRequestException);
    ...
    it('should reject file exceeding size limit for documents (100MB)', async () => {
      ...
      await expect(
        mediaService.uploadMedia(file, MediaType.DOCUMENT, tenantId, userId)
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw if S3 upload fails', async () => {
      const file = createUploadedFile();
      (s3Config.uploadToS3 as jest.Mock).mockRejectedValue(new Error('S3 error'));
    ...
    it('should sanitize S3 key for path traversal attempts', async () => {
      ...
      await mediaService.uploadMedia(file, MediaType.IMAGE, tenantId, userId);
  ...
  describe('getMedia', () => {
    ...
    it('should throw NotFoundException when media not found', async () => {
      ...
      await expect(
        mediaService.getMedia('nonexistent', tenantId)
    ...
    it('should respect tenant isolation', async () => {
      ...
      await expect(
        mediaService.getMedia('media-123', 'different-tenant')
      ).rejects.toThrow(NotFoundException);
  ...
  describe('deleteMedia', () => {
    ...
    it('should throw NotFoundException when media not found', async () => {
      ...
      await expect(
        mediaService.deleteMedia('nonexistent', tenantId)
      ).rejects.toThrow(NotFoundException);
    ...
    it('should continue with database deletion even if S3 fails', async () => {
      mockMediaRepository.findById.mockResolvedValue(mockMedia);
      mockMediaRepository.delete.mockResolvedValue(true);
      (s3Config.deleteFromS3 as jest.Mock).mockRejectedValue(new Error('S3 error'));
  ...
  describe('getPresignedUploadUrl', () => {
    ...
    it('should reject invalid file extension', async () => {
      await expect(
        mediaService.getPresignedUploadUrl(MediaType.IMAGE, 'test.exe', tenantId)
      ).rejects.toThrow(BadRequestException);
    ...
    it('should reject file without extension', async () => {
      await expect(
        mediaService.getPresignedUploadUrl(MediaType.IMAGE, 'noextension', tenantId)
      ).rejects.toThrow(BadRequestException);
    });

    it('should reject mismatched extension for document type', async () => {
      await expect(
        mediaService.getPresignedUploadUrl(MediaType.DOCUMENT, 'image.jpg', tenantId)
      ).rejects.toThrow(BadRequestException);
    });

    it('should reject mismatched extension for video type', async () => {
      await expect(
        mediaService.getPresignedUploadUrl(MediaType.VIDEO, 'image.jpg', tenantId)
      ).rejects.toThrow(BadRequestException);
    });
  });
});
