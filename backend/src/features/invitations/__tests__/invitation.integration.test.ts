...
describe('Invitation Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    container.registerInstance(InvitationService, mockInvitationService);
  ...
  describe('POST /api/invitations - Create Invitation', () => {
    ...
    it('should return 409 for duplicate invitation', async () => {
      mockInvitationService.createInvitation.mockRejectedValue(
        new ConflictException('An invitation for this email is already pending')
      );
    ...
    it('should return 409 when user already exists in tenant', async () => {
      mockInvitationService.createInvitation.mockRejectedValue(
        new ConflictException('A user with this email already exists in this tenant')
      );
  ...
  describe('GET /api/invitations/:token - Validate Invitation', () => {
    it('should return valid invitation details', async () => {
      const validationResult: ValidateInvitationResponseDto = {
        valid: true,
        email: 'invited@example.com',
        tenantName: 'Test Tenant',
        inviterName: 'Admin User',
  ...
  describe('POST /api/invitations/:token/accept - Accept Invitation', () => {
    ...
    it('should return 400 for invalid token', async () => {
      mockInvitationService.acceptInvitation.mockRejectedValue(
        new BadRequestException('Invalid or expired invitation token')
      );
  ...
  describe('POST /api/invitations/:token/decline - Decline Invitation', () => {
    ...
    it('should return 400 for invalid token', async () => {
      mockInvitationService.declineInvitation.mockRejectedValue(
    ...
    it('should return 400 for already processed invitation', async () => {
      mockInvitationService.declineInvitation.mockRejectedValue(
        new BadRequestException('This invitation has already been processed')
      );
  ...
  describe('POST /api/invitations/resend - Resend Invitation', () => {
    ...
    it('should return 404 when no invitation found', async () => {
      mockInvitationService.resendInvitation.mockRejectedValue(
        new NotFoundException('No invitation found for this email')
      );
  ...
  describe('GET /api/invitations - List Invitations', () => {
    ...
    it('should pass pagination parameters to service', async () => {
      ...
      expect(mockInvitationService.getInvitationsByTenant).toHaveBeenCalledWith(
        tenantId,
  ...
  describe('DELETE /api/invitations/:id - Delete Invitation', () => {
    it('should delete invitation successfully', async () => {
      ...
      expect(mockInvitationService.deleteInvitation).toHaveBeenCalledWith(
        tenantId,
    ...
    it('should return 404 when invitation not found', async () => {
      mockInvitationService.deleteInvitation.mockRejectedValue(
