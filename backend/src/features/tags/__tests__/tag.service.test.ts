...
describe('TagService', () => {
  ...
  describe('findById', () => {
    ...
    it('should throw NotFoundException when tag not found', async () => {
      ...
      await expect(
        tagService.findById('nonexistent', tenantId)
    ...
    it('should respect tenant isolation', async () => {
      ...
      await expect(
        tagService.findById('tag-1', 'different-tenant')
      ).rejects.toThrow(NotFoundException);
  ...
  describe('create', () => {
    ...
    it('should throw ConflictException for duplicate name', async () => {
      ...
      await expect(
        tagService.create(createDto, tenantId)
    ...
    it('should allow same name in different tenant', async () => {
      ...
      expect(mockTagRepository.existsByName).toHaveBeenCalledWith(
        createDto.name,
        'different-tenant'
      );
    });
    ...
    it('should create tag with all color options', async () => {
      ...
      for (const color of colorOptions) {
        const dto: CreateTagDto = { name: `Tag-${color}`, color };
        mockTagRepository.existsByName.mockResolvedValue(false);
  ...
  describe('update', () => {
    ...
    it('should throw NotFoundException when tag not found', async () => {
      ...
      await expect(
        tagService.update('nonexistent', updateDto, tenantId)
      ).rejects.toThrow(NotFoundException);
    ...
    it('should throw ConflictException for duplicate name', async () => {
      ...
      await expect(
        tagService.update('tag-1', updateDto, tenantId)
      ).rejects.toThrow(ConflictException);
    ...
    it('should not check duplicate when name unchanged', async () => {
      const dtoSameName: UpdateTagDto = {
    ...
    it('should update only color when name not provided', async () => {
      const dtoColorOnly: UpdateTagDto = {
        color: TagColor.PINK,
      };
    ...
    it('should throw NotFoundException if update returns null', async () => {
      ...
      await expect(
        tagService.update('tag-1', { color: TagColor.RED }, tenantId)
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('delete', () => {
    it('should delete tag successfully', async () => {
      ...
      await expect(
        tagService.delete('tag-1', tenantId)
    ...
    it('should throw NotFoundException when tag not found', async () => {
      ...
      await expect(
        tagService.delete('nonexistent', tenantId)
      ).rejects.toThrow(NotFoundException);
    ...
    it('should respect tenant isolation', async () => {
      ...
      await expect(
        tagService.delete('tag-1', 'different-tenant')
      ).rejects.toThrow(NotFoundException);
