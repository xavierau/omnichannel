...
import { TemplateSseService, TemplateSseEventType } from '../template-sse.service';
...
describe('TemplateSseService', () => {
  let service: TemplateSseService;
  ...
  beforeEach(() => {
    jest.useFakeTimers();
    service = new TemplateSseService();
  });
  ...
  describe('emitToTenant', () => {
    it('should send event to all clients of a tenant', () => {
      ...
      const eventData = { templateName, language, newStatus: 'approved' };
      service.emitToTenant(tenantId, 'template:status:changed', eventData);

      expect(res1.write).toHaveBeenCalledWith('event: template:status:changed\n');
      expect(res1.write).toHaveBeenCalledWith(`data: ${JSON.stringify(eventData)}\n\n`);
      expect(res2.write).toHaveBeenCalledWith('event: template:status:changed\n');
    ...
    it('should not send to clients of different tenant', () => {
      ...
      service.emitToTenant(tenantId, 'template:status:changed', { test: true });
    ...
    it('should handle non-existent tenant gracefully', () => {
      // Should not throw
      expect(() => {
        service.emitToTenant('non-existent-tenant', 'template:status:changed', {});
      }).not.toThrow();
    });
    ...
    it('should not write to ended connections', () => {
      ...
      service.emitToTenant(tenantId, 'template:status:changed', {});
  ...
  describe('emitTemplateStatusChange', () => {
    it('should emit template status change event with correct data', () => {
      const res = createMockResponse();
      service.addClient(tenantId, userId, res);
      ...
      service.emitTemplateStatusChange(
        tenantId,
        templateName,
        language,
        'pending',
      ...
      expect(res.write).toHaveBeenCalledWith('event: template:status:changed\n');

      // Check that data contains all expected fields
      const dataCall = (res.write as jest.Mock).mock.calls.find(
        (call) => call[0].includes('data:')
      );
      expect(dataCall).toBeDefined();

      const writtenData = JSON.parse(dataCall[0].replace('data: ', '').replace('\n\n', ''));
      expect(writtenData.templateName).toBe(templateName);
      expect(writtenData.language).toBe(language);
      expect(writtenData.oldStatus).toBe('pending');
      expect(writtenData.newStatus).toBe('approved');
    ...
    it('should include reason when provided', () => {
      ...
      const reason = 'Template contains prohibited content';
      service.emitTemplateStatusChange(
        tenantId,
        templateName,
        language,
      ...
      const writtenData = JSON.parse(dataCall[0].replace('data: ', '').replace('\n\n', ''));
      expect(writtenData.reason).toBe(reason);
    });

    it('should handle null oldStatus', () => {
      const res = createMockResponse();
      service.addClient(tenantId, userId, res);
      ...
      const writtenData = JSON.parse(dataCall[0].replace('data: ', '').replace('\n\n', ''));
      expect(writtenData.oldStatus).toBeNull();
    });
  });

  describe('emitSyncCompleted', () => {
    it('should emit sync completed event with correct data', () => {
      const res = createMockResponse();
      service.addClient(tenantId, userId, res);
      ...
      service.emitSyncCompleted(tenantId, 15, 'channel-account-123');

      // Check that event type is correct
      expect(res.write).toHaveBeenCalledWith('event: template:sync:completed\n');
      ...
      const writtenData = JSON.parse(dataCall[0].replace('data: ', '').replace('\n\n', ''));
      expect(writtenData.syncedCount).toBe(15);
      expect(writtenData.channelAccountId).toBe('channel-account-123');
      expect(writtenData.timestamp).toBeDefined();
    });

    it('should work without channelAccountId', () => {
      const res = createMockResponse();
      service.addClient(tenantId, userId, res);
      ...
      service.emitSyncCompleted(tenantId, 10);

      const dataCall = (res.write as jest.Mock).mock.calls.find(
        (call) => call[0].includes('data:')
      );
      const writtenData = JSON.parse(dataCall[0].replace('data: ', '').replace('\n\n', ''));
      expect(writtenData.syncedCount).toBe(10);
      expect(writtenData.channelAccountId).toBeUndefined();
    });
  });
  ...
  describe('event types', () => {
    const eventTypes: TemplateSseEventType[] = [
  ...
  describe('concurrency and edge cases', () => {
    ...
    it('should handle multiple tenants simultaneously', () => {
      ...
      service.emitToTenant('tenant-2', 'template:status:changed', {});
