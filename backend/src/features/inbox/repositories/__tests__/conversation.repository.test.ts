...
describe('ConversationRepository - Nested Relations', () => {
  ...
  let mockTypeOrmRepository: jest.Mocked<Repository<Conversation>>;
  ...
  const createMockConversation = (includeChannel = true): Conversation => {
    const conversation = {
      id: conversationId,
      tenantId,
      channelAccountId,
  ...
  beforeEach(() => {
    ...
    jest.spyOn(AppDataSource, 'getRepository').mockReturnValue(mockTypeOrmRepository);
  ...
  describe('findByCustomerAndChannel', () => {
    it('should include channelAccount.channel relation', async () => {
      ...
      const result = await repository.findByCustomerAndChannel(
        tenantId,
        customerId,
        channelAccountId
      );
  ...
  describe('findAllForOperator', () => {
    it('should include channelAccount.channel relation via query builder', async () => {
      ...
      const result = await repository.findAllForOperator(
        tenantId,
        userId,
        [channelAccountId],
        { page: 1, limit: 20 }
    ...
    it('should properly filter by accessible channel accounts', async () => {
      ...
      await repository.findAllForOperator(
        tenantId,
        userId,
        [channelAccountId],
        { page: 1, limit: 20 }
      ...
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith(
        'conversation.channel_account_id IN (:...channelAccountIds)',
