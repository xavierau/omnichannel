...
import { OutgoingWebhookQueue, OutgoingWebhookJobType } from '../../../jobs/outgoing-webhook.queue';
...
describe('OutgoingWebhookQueue', () => {
  let queue: OutgoingWebhookQueue;
  let mockDispatcher: jest.Mocked<OutgoingWebhookDispatcher>;
  ...
  const createMockJobData = (overrides: Partial<WebhookDispatchJobData> = {}): WebhookDispatchJobData => ({
  ...
  beforeEach(() => {
    ...
    } as unknown as jest.Mocked<OutgoingWebhookDispatcher>;

    queue = new OutgoingWebhookQueue(mockDispatcher);
    mockBullQueue = queue.getQueue() as jest.Mocked<Bull.Queue>;
  });
  ...
  describe('getQueueStats', () => {
    it('should return queue statistics', async () => {
      (mockBullQueue.getJobCounts as jest.Mock).mockResolvedValue({
        waiting: 5,
        active: 2,
        completed: 100,
        failed: 3,
