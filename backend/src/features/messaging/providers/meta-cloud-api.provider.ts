...
export interface MetaTemplateComponent {
  ...
  example?: {
    header_text?: string[];
    header_handle?: string[];  // For media headers (public URL)
    body_text?: string[][];
  };
}
...
export class MetaCloudApiProvider implements IMessagingProvider {
   ...
   */
  async createTemplate(request: CreateMetaTemplateRequest): Promise<CreateMetaTemplateResponse> {
    if (!this.client || !this.credentials) {
      return {
        success: false,
        error: {
    ...
    try {
      logger.debug('Creating template on Meta', {
        name: request.name,
      ...
      logger.info('Template created successfully on Meta', {
  ...
  private handleCreateTemplateError(
    error: unknown,
    request: CreateMetaTemplateRequest
    ...
    logger.error('Failed to create template on Meta', {
      errorCode,
      errorMessage,
      retryable,
      templateName: request.name,
