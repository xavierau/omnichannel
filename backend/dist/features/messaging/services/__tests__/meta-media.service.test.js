...
const meta_media_service_1 = require("../meta-media.service");
// Mock the logger
...
describe('MetaMediaService', () => {
    ...
    beforeEach(() => {
        ...
        metaMediaService = new meta_media_service_1.MetaMediaService(mockProviderFactory);
        // Default S3 mock implementations
        s3Config.uploadToS3.mockResolvedValue(undefined);
        s3Config.getDownloadSignedUrl.mockResolvedValue('https://s3.example.com/signed-url');
    });
    describe('processInboundMedia', () => {
        it('should download image from Meta and upload to S3', async () => {
            ...
            const result = await metaMediaService.processInboundMedia(mediaId, mimeType, channelAccountId, tenantId, conversationId);
            // Verify provider factory was called
            expect(mockProviderFactory.createProviderForChannelAccount).toHaveBeenCalledWith(channelAccountId);
            ...
            expect(s3Config.uploadToS3).toHaveBeenCalledWith(expect.stringContaining(`${tenantId}/media/${conversationId}/`), mediaBuffer, mimeType);
            // Verify the S3 key has correct extension
            const uploadCall = s3Config.uploadToS3.mock.calls[0];
            expect(uploadCall[0]).toMatch(/\.jpg$/);
            // Verify signed URL was generated
            expect(s3Config.getDownloadSignedUrl).toHaveBeenCalledWith(expect.stringContaining(`${tenantId}/media/${conversationId}/`), 7 * 24 * 60 * 60 // 7 days in seconds
            );
        ...
        it('should use downloaded content type when different from provided MIME type', async () => {
            ...
            const result = await metaMediaService.processInboundMedia(mediaId, providedMimeType, channelAccountId, tenantId, conversationId);
            ...
            expect(s3Config.uploadToS3).toHaveBeenCalledWith(expect.any(String), mediaBuffer, actualMimeType);
        });
        it('should throw error when provider does not support media operations', async () => {
            ...
            mockProviderFactory.createProviderForChannelAccount.mockResolvedValue(providerWithoutMedia);
            await expect(metaMediaService.processInboundMedia(mediaId, 'image/jpeg', channelAccountId, tenantId, conversationId)).rejects.toThrow('Provider does not support media operations');
        });
        it('should propagate error when getMediaUrl fails', async () => {
            mockProvider.getMediaUrl.mockRejectedValue(new Error('Failed to get media URL: Invalid media ID'));
            await expect(metaMediaService.processInboundMedia(mediaId, 'image/jpeg', channelAccountId, tenantId, conversationId)).rejects.toThrow('Failed to get media URL');
        ...
        it('should propagate error when downloadMedia fails', async () => {
            mockProvider.getMediaUrl.mockResolvedValue('https://cdn.meta.com/media/123');
            mockProvider.downloadMedia.mockRejectedValue(new Error('Failed to download media: Connection timeout'));
            await expect(metaMediaService.processInboundMedia(mediaId, 'image/jpeg', channelAccountId, tenantId, conversationId)).rejects.toThrow('Failed to download media');
        ...
        it('should propagate error when S3 upload fails', async () => {
            ...
            s3Config.uploadToS3.mockRejectedValue(new Error('S3 upload failed'));
            await expect(metaMediaService.processInboundMedia(mediaId, mimeType, channelAccountId, tenantId, conversationId)).rejects.toThrow('S3 upload failed');
        });
        it('should generate unique S3 keys for each upload', async () => {
            ...
            const result1 = await metaMediaService.processInboundMedia(mediaId, mimeType, channelAccountId, tenantId, conversationId);
            const result2 = await metaMediaService.processInboundMedia(mediaId, mimeType, channelAccountId, tenantId, conversationId);
        ...
        it('should log debug message when processing media', async () => {
            ...
            await metaMediaService.processInboundMedia(mediaId, mimeType, channelAccountId, tenantId, conversationId);
            expect(logger_config_1.logger.debug).toHaveBeenCalledWith('Processing inbound media', expect.objectContaining({
                mediaId,
                mimeType,
                channelAccountId,
                tenantId,
        ...
        it('should log info message after successful upload', async () => {
            ...
            await metaMediaService.processInboundMedia(mediaId, mimeType, channelAccountId, tenantId, conversationId);
            expect(logger_config_1.logger.info).toHaveBeenCalledWith('Inbound media processed and uploaded to S3', expect.objectContaining({
                mediaId,
    ...
    describe('refreshMediaUrl', () => {
        it('should generate new signed URL for existing S3 key', async () => {
            ...
            expect(s3Config.getDownloadSignedUrl).toHaveBeenCalledWith(s3Key, 7 * 24 * 60 * 60 // Default 7 days
            );
