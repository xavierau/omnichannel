...
describe('MessagingRateLimiterService', () => {
    ...
    beforeEach(() => {
        ...
        rateLimiterService = new rate_limiter_service_1.MessagingRateLimiterService(mockRedis);
    });
    afterEach(() => {
        jest.useRealTimers();
    });
    describe('checkRateLimit', () => {
        ...
        it('should use the correct Redis key pattern', async () => {
            ...
            expect(mockPipeline.zremrangebyscore).toHaveBeenCalledWith(`rate_limit:messaging:${channelAccountId}`, 0, expect.any(Number));
        });
    ...
    describe('handleRateLimitError', () => {
        it('should set backoff key with provided retry-after seconds', async () => {
            ...
            expect(mockRedis.setex).toHaveBeenCalledWith(`rate_limit:backoff:${channelAccountId}`, 120, 'true');
        });
        it('should use default backoff of 60 seconds when retry-after not provided', async () => {
            ...
            expect(mockRedis.setex).toHaveBeenCalledWith(`rate_limit:backoff:${channelAccountId}`, 60, 'true');
        });
    });
    describe('isInBackoff', () => {
        it('should return true when backoff key exists', async () => {
            ...
            expect(result).toBe(true);
            expect(mockRedis.get).toHaveBeenCalledWith(`rate_limit:backoff:${channelAccountId}`);
        });
    ...
    describe('clearBackoff', () => {
        it('should remove the backoff key', async () => {
            ...
            expect(mockRedis.del).toHaveBeenCalledWith(`rate_limit:backoff:${channelAccountId}`);
        });
    });
    ...
    describe('custom configuration', () => {
        it('should use custom rate limit when provided', async () => {
            ...
            const baseService = new rate_limiter_service_1.MessagingRateLimiterService(mockRedis);
