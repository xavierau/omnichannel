...
const meta_cloud_api_provider_1 = require("../meta-cloud-api.provider");
// Mock axios
jest.mock('axios');
...
describe('MetaCloudApiProvider', () => {
    ...
    beforeEach(() => {
        ...
        provider = new meta_cloud_api_provider_1.MetaCloudApiProvider();
    });
        ...
        describe('when provider is initialized', () => {
            ...
            describe('text messages', () => {
                it('should send a text message successfully', async () => {
                    ...
                    expect(mockClient.post).toHaveBeenCalledWith('/123456789/messages', expect.objectContaining({
                        messaging_product: 'whatsapp',
            ...
            describe('unsupported content type', () => {
                it('should throw error for unsupported content type', async () => {
                    const request = {
                        recipient: '+1234567890',
                        contentType: 'unknown_type', // Invalid type
                        content: { text: 'test' },
                    };
                    const result = await provider.sendFreeformMessage(request);
                    expect(result.success).toBe(false);
    ...
    describe('getMediaUrl', () => {
        describe('when provider is not initialized', () => {
            it('should throw error when not initialized', async () => {
                await expect(provider.getMediaUrl('media-123')).rejects.toThrow('Provider not initialized. Call initialize() first.');
            });
        });
        describe('when provider is initialized', () => {
            beforeEach(async () => {
            ...
            it('should throw error when API call fails', async () => {
                ...
                await expect(provider.getMediaUrl(mediaId)).rejects.toThrow('Failed to get media URL: Media not found');
            });
            it('should handle network errors gracefully', async () => {
                const mediaId = 'media-123';
                const networkError = new Error('Network Error');
                mockClient.get.mockRejectedValue(networkError);
                await expect(provider.getMediaUrl(mediaId)).rejects.toThrow('Failed to get media URL: Network Error');
            });
        });
    });
    describe('downloadMedia', () => {
        describe('when provider is not initialized', () => {
            it('should throw error when not initialized', async () => {
                await expect(provider.downloadMedia('https://cdn.meta.com/media/123')).rejects.toThrow('Provider not initialized. Call initialize() first.');
            });
        });
        describe('when provider is initialized', () => {
            beforeEach(async () => {
            ...
            it('should download media successfully', async () => {
                ...
                expect(mockedAxios.get).toHaveBeenCalledWith(mediaUrl, expect.objectContaining({
                    headers: {
            ...
            it('should throw error when download fails', async () => {
                ...
                await expect(provider.downloadMedia(mediaUrl)).rejects.toThrow('Failed to download media: Request failed with status code 410');
            });
            it('should handle timeout errors', async () => {
                const mediaUrl = 'https://cdn.meta.com/media/slow';
                const timeoutError = new Error('timeout of 60000ms exceeded');
                mockedAxios.get.mockRejectedValue(timeoutError);
                await expect(provider.downloadMedia(mediaUrl)).rejects.toThrow('Failed to download media: timeout of 60000ms exceeded');
            });
