...
var TemplateSseService_1;
Object.defineProperty(exports, "__esModule", { value: true });
exports.TemplateSseService = void 0;
 ...
 */
let TemplateSseService = class TemplateSseService {
    static { TemplateSseService_1 = this; }
    ...
    constructor() {
        this.emitter.setMaxListeners(TemplateSseService_1.MAX_LISTENERS);
    }
    /**
    ...
    addClient(tenantId, userId, res) {
        ...
        if (tenantClientCount >= TemplateSseService_1.MAX_CONNECTIONS_PER_TENANT) {
            this.sendErrorAndClose(res, 503, 'Too many connections for this tenant');
            logger_config_1.logger.warn('Template SSE connection rejected: tenant limit reached', {
                tenantId,
                currentCount: tenantClientCount,
                limit: TemplateSseService_1.MAX_CONNECTIONS_PER_TENANT,
            });
            return false;
        }
        // Check per-user connection limit
        const userConnectionCount = this.userConnections.get(userId) ?? 0;
        if (userConnectionCount >= TemplateSseService_1.MAX_CONNECTIONS_PER_USER) {
            this.sendErrorAndClose(res, 503, 'Too many SSE connections for this user');
            logger_config_1.logger.warn('Template SSE connection rejected: user limit reached', {
                userId,
                currentCount: userConnectionCount,
                limit: TemplateSseService_1.MAX_CONNECTIONS_PER_USER,
            });
            return false;
        }
        // Set SSE headers
        ...
        }, TemplateSseService_1.HEARTBEAT_INTERVAL_MS);
        // Create client entry
        const client = {
            res,
            heartbeatInterval,
        ...
        res.on('close', () => {
            ...
            logger_config_1.logger.debug('Template SSE client disconnected', { tenantId, userId });
        });
        logger_config_1.logger.debug('Template SSE client connected', { tenantId, userId });
        return true;
    }
    /**
    ...
    emitToTenant(tenantId, eventType, data) {
        ...
        logger_config_1.logger.debug('Template SSE event emitted', {
            tenantId,
            eventType,
     ...
     */
    emitTemplateStatusChange(tenantId, templateName, language, oldStatus, newStatus, reason) {
        const eventData = {
            templateName,
            language,
            oldStatus,
        ...
        logger_config_1.logger.info('Template status change SSE event emitted', {
            tenantId,
            templateName,
            language,
            oldStatus,
     ...
     */
    emitSyncCompleted(tenantId, syncedCount, channelAccountId) {
        const eventData = {
            syncedCount,
            channelAccountId,
            timestamp: new Date().toISOString(),
        ...
        logger_config_1.logger.info('Template sync completed SSE event emitted', {
            tenantId,
            syncedCount,
            channelAccountId,
        });
    ...
    closeAllConnections(tenantId) {
        ...
        if (clientSet) {
            ...
            this.clients.delete(tenantId);
            logger_config_1.logger.debug('All template SSE connections closed', { tenantId });
        }
    }
};
exports.TemplateSseService = TemplateSseService;
exports.TemplateSseService = TemplateSseService = TemplateSseService_1 = __decorate([
