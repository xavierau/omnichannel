...
describe('ApiKeyRepository', () => {
    ...
    beforeEach(() => {
        ...
        database_config_1.AppDataSource.getRepository.mockReturnValue(mockTypeOrmRepo);
        // Create repository instance
        repository = new api_key_repository_1.ApiKeyRepository();
    });
    describe('create', () => {
        ...
        it('should create an API key with expiration date', async () => {
            ...
            mockTypeOrmRepo.create.mockReturnValue(expiringKey);
            mockTypeOrmRepo.save.mockResolvedValue(expiringKey);
            const result = await repository.create(createParams);
        ...
        it('should create an API key scoped to a channel account', async () => {
            ...
            mockTypeOrmRepo.create.mockReturnValue(scopedKey);
            mockTypeOrmRepo.save.mockResolvedValue(scopedKey);
            const result = await repository.create(createParams);
    ...
    describe('findAllByTenant', () => {
        it('should find all API keys for a tenant', async () => {
            ...
            mockTypeOrmRepo.find.mockResolvedValue(apiKeys);
    ...
    describe('updateLastUsedAt', () => {
        it('should update the lastUsedAt timestamp', async () => {
            ...
            expect(mockTypeOrmRepo.update).toHaveBeenCalledWith(apiKeyId, expect.objectContaining({
                lastUsedAt: expect.any(Date),
            }));
            // Verify the date is within expected range
            const updateCall = mockTypeOrmRepo.update.mock.calls[0];
            const updatedDate = updateCall[1].lastUsedAt;
    ...
    describe('deactivate', () => {
        it('should deactivate an API key within a tenant', async () => {
            ...
            mockTypeOrmRepo.findOne.mockResolvedValue(mockApiKey);
            mockTypeOrmRepo.save.mockResolvedValue(deactivatedKey);
            const result = await repository.deactivate(tenantId, apiKeyId);
            expect(mockTypeOrmRepo.findOne).toHaveBeenCalledWith({
                where: { id: apiKeyId, tenantId },
            });
            expect(mockTypeOrmRepo.save).toHaveBeenCalledWith(expect.objectContaining({ isActive: false }));
