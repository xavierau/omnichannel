...
describe('WebhookService', () => {
    ...
    let mockMessageLogRepo;
    let mockChannelAccountRepo;
    let mockTemplateRepo;
    let mockTemplateSseService;
    let mockCredentialService;
    let mockInboxMessageQueue;
    ...
    beforeEach(() => {
        ...
        webhookService = new webhook_service_1.WebhookService(mockProviderFactory, mockMessageLogRepo, mockChannelAccountRepo, mockTemplateRepo, mockTemplateSseService, mockCredentialService, mockInboxMessageQueue);
    });
    describe('processInboundMessage', () => {
        const createMessageReceivedEvent = (messageType, content, phoneNumberId = '123456789') => ({
        ...
        describe('when phone_number_id is present', () => {
            it('should queue inbound text message for processing', async () => {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                await webhookService.processInboundMessage(event);
                expect(mockChannelAccountRepo.findByPhoneNumberId).toHaveBeenCalledWith('123456789');
                expect(mockInboxMessageQueue.queueInboundProcessing).toHaveBeenCalledWith(expect.objectContaining({
                    tenantId: 'tenant-123',
    ...
    describe('extractMessageContent', () => {
        it('should extract text content', () => {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const content = webhookService.extractMessageContent(rawMessage);
    ...
    describe('processWebhookEvent with message_received', () => {
        it('should route message_received events to processInboundMessage', async () => {
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            mockProviderFactory.createProviderForWebhook.mockReturnValue(mockProvider);
            const channelAccount = createMockChannelAccount();
            mockChannelAccountRepo.findByPhoneNumberId.mockResolvedValue(channelAccount);
            ...
            try {
                const result = await webhookService.processMetaWebhook(JSON.stringify({ object: 'whatsapp_business_account', entry: [] }), 'sha256=valid-signature');
