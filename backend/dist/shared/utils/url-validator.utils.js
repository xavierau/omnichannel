...
exports.SsrfValidationError = void 0;
 ...
 */
async function validateWebhookUrl(urlString, allowHttp = false) {
    let url;
    // Parse the URL
    try {
    ...
    try {
        ...
        for (const ip of ipv4Addresses) {
            if (isPrivateIPv4(ip)) {
                logger_config_1.logger.warn('Webhook URL resolves to private IPv4', {
                    hostname,
                    resolvedIp: ip,
                });
                return {
        ...
        for (const ip of ipv6Addresses) {
            if (isPrivateIPv6(ip)) {
                logger_config_1.logger.warn('Webhook URL resolves to private IPv6', {
                    hostname,
                    resolvedIp: ip,
                });
                return {
        ...
        logger_config_1.logger.error('DNS resolution error during webhook URL validation', {
            hostname,
            error: errorMessage,
        });
        return {
...
class SsrfValidationError extends Error {
    url;
    resolvedIp;
    constructor(message, url, resolvedIp) {
        super(message);
        this.url = url;
...
exports.SsrfValidationError = SsrfValidationError;
