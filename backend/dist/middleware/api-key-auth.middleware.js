...
exports.validateChannelAccountScope = validateChannelAccountScope;
...
async function apiKeyAuth(req, res, next) {
    try {
        ...
        const apiKeyService = tsyringe_1.container.resolve(api_key_service_1.ApiKeyService);
        const apiKey = await apiKeyService.validateKey(rawKey);
        if (!apiKey) {
            logger_config_1.auditLogger.warn('API key authentication failed: invalid key', {
                path: req.path,
                method: req.method,
            });
        ...
        apiKeyService.recordUsage(apiKey.id).catch((err) => {
            ...
            logger_config_1.auditLogger.warn('Failed to record API key usage', {
                apiKeyId: apiKey.id,
                error: err.message,
            });
        });
...
function requireApiKeyPermission(...permissions) {
    return async (req, res, next) => {
        ...
        if (!hasPermission) {
            logger_config_1.auditLogger.warn('API key permission denied', {
                apiKeyId: apiKey.id,
 ...
 * );
 */
async function validateChannelAccountScope(req, res, next) {
    const apiKey = req.apiKey;
    if (!apiKey) {
        sendUnauthorized(res, ErrorMessages.AUTH_REQUIRED);
        return;
    ...
    const targetChannelAccountId = req.params.channelAccountId || req.body?.channelAccountId;
    ...
    if (apiKey.channelAccountId !== targetChannelAccountId) {
        logger_config_1.auditLogger.warn('API key channel scope violation', {
            apiKeyId: apiKey.id,
            tenantId: apiKey.tenantId,
